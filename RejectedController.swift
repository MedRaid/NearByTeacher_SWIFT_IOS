...
class RejectedController: UIViewController, UITableViewDataSource, UITableViewDelegate {
        ...
        _ = FIRDatabase.database().reference(withPath: "requests").queryOrdered(byChild: "Receiver").queryEqual(toValue: hismail).observeSingleEvent(of: .value, with: { (snapshot) in
            if let snapshots = snapshot.children.allObjects as? [FIRDataSnapshot] {
                ...
                if (self.items.isEmpty)
                {
                    ...
                    let alert = UIAlertController(title: "Guess What", message: "No Rejected requests for the moment", preferredStyle: UIAlertControllerStyle.alert)
                    // add an action (button)
                    alert.addAction(UIAlertAction(title: "OK", style: UIAlertActionStyle.default, handler: nil))
                    alert.addAction(UIAlertAction(title: "Cancel", style: UIAlertActionStyle.cancel, handler: nil))
