...
//        
//        var error: Fault?
//        let bc = Backendless.sharedInstance().data.of(Requestsi.ofClass()).find(dataQuery, fault: &error)
//        if error == nil {
//            
//            
//            let currentPage = bc?.getCurrentPage()
//            for teacher in currentPage as! [Requestsi] {
//                
//                
//                print(teacher.Receiver?.getProperty("pic")! as Any)
...
//        
            ...
            if let snapshots = snapshot.children.allObjects as? [FIRDataSnapshot] {
                ...
                if (self.items.count-1 > 0){
                    for i in 0...self.items.count-1
                    {
                        ...
                        if (self.items.count-1 > 0)
                        {
                          //  if (self.items[i].status == "0" || self.items[i].status == "1" || self.items[i].status == "2"){
                ...
                if (self.items.isEmpty){
                    ...
                    let alert = UIAlertController(title: "Check", message: "No Pending Requests Yet", preferredStyle: UIAlertControllerStyle.alert)
                    alert.addAction(UIAlertAction(title: "Cancel", style: UIAlertActionStyle.default, handler: nil))
    ...
    func setupHeaderView() {
        ...
        label.text = " Your Pending Requests"
...
//        }
    ...
    public override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let reqs = self.itemsaf[indexPath.row]
        let vc = RequestDetailViewControllers()
        vc.id = tteacher
        vc.req = reqs
